    ...
    <div className="space-y-8 pb-12">
      ...
      <div className="space-y-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-black text-white uppercase italic tracking-tighter">Recent Provisions</h2>
            <p className="text-white/30 text-[10px] font-bold uppercase tracking-widest">Global Resource Allocation Log</p>
          </div>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-white/5 bg-white/[0.02]">
                <th className="px-6 py-5 text-[10px] font-black uppercase tracking-widest text-white/30 italic">Asset Serial</th>
                <th className="px-6 py-5 text-[10px] font-black uppercase tracking-widest text-white/30 italic">Inventory Name</th>
                <th className="px-6 py-5 text-[10px] font-black uppercase tracking-widest text-white/30 italic">Personnel</th>
            ...
            <tbody className="divide-y divide-white/[0.02]">
              {recentAssignments.map((item) => (
                <tr key={item.id} className="group hover:bg-white/[0.03] transition-colors">
                  ...
                  <td className="px-6 py-5">
                    <span className="text-sm font-medium text-white/60 group-hover:text-white transition-colors italic">{item.employee_name}</span>
                  </td>
                  <td className="px-6 py-5">
                    <span className={`px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest border ${
                      item.status.toUpperCase() === 'ACTIVE' || item.status.toUpperCase() === 'DEPLOYED' 
              ...
              {recentAssignments.length === 0 && (
                <tr>
                  <td colSpan={4} className="px-6 py-12 text-center text-white/10 italic text-xs font-black uppercase tracking-[0.3em]">
                    No active provisions detected in local cache
                  </td>
                </tr>
              )}
            </tbody>
